...
oc set volume deployment mongodb-community-server --remove --mount-path=/data/db --confirm

oc set volumes deployment mongodb-community-server --add --mount-path=/data/db --name=mongopvc --claim-name=mongopvc --read-only=false --type=persistentVolumeClaim --claim-size=1Gi
